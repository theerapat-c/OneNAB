...
SELECT  HANAFlag
	  ...
	  , BType AS BillType
	  , CASE WHEN BType = 'TaxInv' THEN COUNT(DISTINCT Documents) ELSE 0 END AS NoOfBill
	  , CASE WHEN BType <> 'TaxInv' THEN COUNT(DISTINCT Documents) ELSE 0 END AS NoOfBillCN
	  , CASE WHEN BType = 'TaxInv' THEN COUNT(DISTINCT CONCAT(BasicMaterial,Documents)) ELSE 0 END AS NoOfBrand
	  , CASE WHEN BType <> 'TaxInv' THEN COUNT(DISTINCT CONCAT(BasicMaterial,Documents)) ELSE 0 END AS NoOfBrandCN
...
-- DROP TABLE SSC_AGG_Invoice_Bill
-- INTO SSC_AGG_Invoice_Bill
...
GROUP BY HANAFlag, CONCAT(YM,'01'), Branch, Route, Salesman, Customer, ChannelMarge, DataSource, BType
ORDER BY HANAFlag, CONCAT(YM,'01'), Branch, Route, Salesman, Customer, ChannelMarge, DataSource
